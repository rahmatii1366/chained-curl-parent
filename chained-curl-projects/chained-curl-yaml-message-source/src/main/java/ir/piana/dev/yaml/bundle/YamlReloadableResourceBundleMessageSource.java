 ...
 * @see ResourceBundle
 */
public class YamlReloadableResourceBundleMessageSource extends AbstractResourceBasedMessageSource
    ...
    private YamlPropertiesPersister propertiesPersister = YamlPropertiesPersister.INSTANCE;
    ...
    protected PropertiesHolder refreshProperties(String filename, @Nullable PropertiesHolder propHolder) {
        ...
        Resource resource = this.resourceLoader.getResource(filename + YAML_SUFFIX);
        if(!resource.exists()) {
    ...
    protected Properties loadProperties(Resource resource, String filename) throws IOException {
        ...
        if (resourceFilename != null && !resourceFilename.endsWith(YAML_SUFFIX)) {
            if (logger.isDebugEnabled()) {
                logger.debug("Loading properties [" + resource.getFilename() + "]");
            }
            try (InputStream is = resource.getInputStream()) {
                if (resourceFilename.endsWith(XML_SUFFIX))
                    this.propertiesPersister.loadFromXml(props, is);
                else {
                    String encoding = getEncoding(resource, filename);
                    if (encoding != null) {
                        if (logger.isDebugEnabled()) {
            ...
            String encoding = getEncoding(resource, filename);
            logger.debug("Loading properties [" + resource.getFilename() + "]" +
                    encoding == null ? "" : " with encoding '" + encoding + "'");

            if (resource == null)
                this.propertiesPersister.load(props, filename);
            else
                this.propertiesPersister.load(props, resource);
        }
        return props;
    }

    private String getEncoding(Resource resource, String filename) {
     ...
     */
    public void setPropertiesPersister(@Nullable YamlPropertiesPersister propertiesPersister) {
        this.propertiesPersister =
                (propertiesPersister != null ? propertiesPersister : YamlPropertiesPersister.INSTANCE);
    }
