...
@Component
public class LenBetweenValidator implements Validator<String> {
    @Override
    ...
    public List<String> commandNames() {
        return Arrays.asList("length-between", "lb");
    }

    @Override
    public boolean validate(String value, String... options) {
        return (value.length() >= Integer.valueOf(options[0]) && value.length() <= Integer.valueOf(options[1]));
    }
}
